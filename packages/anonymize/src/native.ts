...
import type { NerConfig } from "./gliner2/apply-ner";
import { applyNerToResult } from "./gliner2/apply-ner";
...
export class PreparedNativePipeline {
  ...
  readonly #nerConfig?: NerConfig | undefined;

  constructor(anonymizer: PreparedNativeAnonymizer, nerConfig?: NerConfig) {
  ...
  /**
   * Async redaction that additionally runs the configured NER backend
   * (`nerInference`) and overlays detected spans onto the Rust result. Use
   * this instead of `redactText` when `enableNer` is set; the synchronous
   * `redactText` runs Rust detectors only.
   */
  async redactTextWithNer(
    fullText: string,
    operators?: NativeOperatorConfig,
  ): Promise<NativeStaticRedactionResult> {
    const nerConfig = this.#nerConfig;
    const base = this.#anonymizer.redactStaticEntities(fullText, operators);
    if (nerConfig === undefined) {
      return base;
    }
    return applyNerToResult(base, fullText, nerConfig, operators);
  }
