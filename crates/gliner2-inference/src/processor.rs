    ...
    pub fn transform(&self, text: &str, schema_tasks: &[SchemaTask]) -> Result<ProcessedRecord> {
        ...
        for (i, token) in combined_tokens.iter().enumerate() {
            ...
            if i >= text_start_idx && i < text_end_idx {
                word_to_token_maps.push((
                    current_subword_idx - ids.len(),
                    current_subword_idx,
                ));
            }
        }
        ...
        Ok(ProcessedRecord {
            ...
            tasks,
            text_start: word_to_token_maps.first().map(|v| v.0).unwrap_or(0),
            text_end: word_to_token_maps.last().map(|v| v.1).unwrap_or(0),
            word_to_token_maps,
            word_to_char_maps,
        })
    }
